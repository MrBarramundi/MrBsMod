package com.mrbarramundi.mrbsmod.client.gui.screen.inventory;
 ...
 */
@OnlyIn(Dist.CLIENT)
public class DiamondBladedStonecutterScreen extends ContainerScreen<DiamondBladedStonecutterContainer>
{
    private static final ResourceLocation BACKGROUND_TEXTURE = new ResourceLocation(MrBsMod.MOD_ID, "textures/gui/container/diamond_bladed_stonecutter.png");
    private float sliderProgress;
    private boolean clickOnScrollBar;
    private int recipeIndexOffset;
    private boolean hasItemsInInputSlot;

    public DiamondBladedStonecutterScreen(DiamondBladedStonecutterContainer container, PlayerInventory playerInventory, ITextComponent titleIn)
    {
    ...
    {
        List<DiamondBladedStonecuttingRecipe> list = this.container.getRecipeList();
        for(int i = this.recipeIndexOffset; i < indexOffset && i < this.container.getRecipeListSize(); ++i)
        {
            int index = i - this.recipeIndexOffset;
            int recipeOffsetX = offsetX + index % 4 * 16;
